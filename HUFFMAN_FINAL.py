...
class NodeTree(object):
    def __init__(self, left=None, right=None, frequency=None):
        self.left = left
        self.right = right
...
while len(nodes) > 1:
    ...
    nodes = nodes[:-2]
    frequency = c1 + c2
    node = NodeTree(key1, key2, frequency)
    nodes.append((node, frequency))
    total_size_of_characters += frequency * len(huffman_code_tree(node))
...
def generate_dot_file(node, dot, parent_node=None):
    if type(node) is str:
        dot.node(node, color='red')  # Change node color to red
        if parent_node:
            dot.edge(parent_node, node, color='blue')  # Change edge color to blue
    else:
        left, right = node.children()
        dot.node(str(node), color='red')  # Change node color to red
        if parent_node:
            dot.edge(parent_node, str(node), label=str(node.frequency), color='blue')  # Change edge color to blue
